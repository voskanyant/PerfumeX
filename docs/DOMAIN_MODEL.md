...
- Fragrantica product - staged external catalogue row from saved HTML import; evidence, not canonical data. Model: `assistant_linking.FragranticaProduct`.
...
1. Parse saved HTML into staged `FragranticaProduct` rows.
...
5. Create catalogue facts or aliases only from reviewed/approved work.
...
- `FragranticaProduct` is not trusted catalogue data. It is staged external evidence.
