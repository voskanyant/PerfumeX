...
4. Review staged rows beside local catalogue rows. Suggested matches should be ranked and should use catalogue brand/product aliases, loose punctuation keys, audience synonyms such as `man`/`men` and `woman`/`women`, and fuzzy same-brand scent scoring; do not limit suggestions to exact normalized names.
