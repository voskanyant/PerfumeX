...
5. Treat damaged/no-cellophane packaging phrases as structured dented packaging terms (`parser_dented_packaging_term`), not as garbage exclusions. Treat positive supplier cap phrases such as `с крышкой` and abbreviations such as `c фирм. крыш` as `With Cap` packaging terms, not removable comments or scent-name tokens. Treat old-design abbreviations such as `ст.ди` as structured packaging terms. Treat supplier `стекло` notes as no-box tester-bottle metadata: strip the token from the scent name, set packaging to `No Box`, and mark the row as `Tester`. After any type/modifier detection such as sample/tester/travel/set/decoded, strip the detected marker from `product_name_text`; terms like `пробник` are variant metadata, not scent names.
...
8. Treat Cyrillic supplier concentration abbreviations such as `тв`, `т/в`, and `т.в.` as `Eau de Toilette`, and strip them from scent names just like `edt`.
9. Treat multiple supplier size expressions in one row, such as `50ml + 10ml`, `50ml 10ml`, `100ml EDP + 30ml EDP`, compact `100edp+30edp`, `2*10ml`, or `2x10ml`, as a set. Strip all size tokens from the scent name, keep a combined display size, and mark the parsed variant as `Set`.
10. Treat supplier-only descriptor/color comments such as Cyrillic `белый` as `parser_supplier_comment_term` text that is stripped from the scent name without changing type or packaging.
11. Treat a supplier row name ending in a standalone `*` as a fake/garbage marker and exclude it from normal product parsing.
12. Distinguish refill products from refillable bottles: `refill`/`refil` means the product is a refill, while `refillable` is packaging/comment-style metadata and must not become part of the scent name or refill type.
13. Normalize Cyrillic/Latin lookalike letters inside otherwise Latin supplier tokens before parsing, so mixed-script scent names such as `Ciel` keep their Latin identity while fully Cyrillic supplier notes stay available to Russian term rules.
14. Treat short supplier audience markers such as `wom` as parser audience knowledge. Treat supplier `L` as Woman only when it appears as an isolated marker such as `(L)` or `[L]`, not when it is part of a scent word. When the catalogue has same-base men/women scents, keep the audience in the parsed scent name (`Guilty Woman`/`Guilty Man` or the catalogue's equivalent names).
15. Expand supplier abbreviations such as `Limited Ed.` to canonical name suffixes such as `Limited Edition`; when the supplier gives an audience hint and the catalogue has audience-specific base scents, keep that audience in the canonical scent name before the suffix.
16. Treat supplier collection-looking prefixes as untrusted unless backed by a brand-scoped collection alias; approved collection aliases may infer the brand when the supplier omitted it. Use product aliases when supplier text incorrectly prepends a collection name to a scent, preserve intentional uppercase suffixes such as `WB`, and canonicalize supplier-expanded short names such as `Saint Honore` to catalogue style `St Honore`. Generic or standalone trailing brand words such as `collection` or `privee` must not become generated brand aliases because they can falsely map unrelated supplier rows to brands like `Attar Collection` or `Salle Privee`; teach real short supplier brand text as explicit `BrandAlias` rows. Generic one-word product aliases such as `Woman`, `Leather`, or `Oud` must not infer a missing brand; they may apply only after the brand is already known. Product aliases may infer a missing brand only when the alias starts the remaining supplier identity and is explicit/high-signal, not when a scent phrase appears later after unknown leading text. Bare `attar` at the beginning of a supplier row is brand evidence for `Attar Collection`, not a concentration alias for `Perfume Oil`; explicit concentration text such as `edp` must remain authoritative.
17. Treat supplier spelling `HB Boss` as a Hugo Boss/Boss brand alias, not as scent text. Example: `HB Boss Jour Runway Edition W edp 50 ml` should parse brand as Hugo Boss, scent as `Jour Runway Edition`, audience as Woman, concentration as Eau de Parfum, and size as 50ml.
17. Treat unresolved Cyrillic brand or scent identity text as manual review unless it is consumed by an approved brand/product alias or known parser term. Do not let an inner Latin word later in the supplier row become the brand when the row starts with unresolved Cyrillic identity text.
18. Treat supplier rows containing Cyrillic `по мотивам` as inspired-by/imitation rows and route them to garbage/exclusion instead of matching them to the referenced real brand.
19. Treat supplier atomizer words such as `atomiser`, `atomizer`, and Cyrillic `атомайзер` as `parser_atomizer_term` knowledge. Atomizer rows are their own assistant category and should display `/ Atomizer` in the parsed identity while stripping the supplier atomizer marker from the scent name.
20. Catalogue-confirmed scent identity can contain words that are also parser metadata terms. If an exact same-brand catalogue scent match uses a term such as `Vintage`, preserve it in the scent name and do not route the row to that variant category. If the supplier brand field duplicates the raw leading brand, strip the repeated leading brand before scent extraction. Example: `M. Micallef Royal Vintage m edp100ml` parses as brand `M. Micallef`, scent `Royal Vintage`, audience `Men`, `Eau de Parfum`, `100ml`.
21. Treat a recognized leading brand as supplier identity source of truth. If a parenthetical supplier comment contains another known brand, strip that comment from the scent, keep the leading brand, and mark the row for manual review. Example: `AlHambra Tonic Ice (GNTONIC M. Micallef) edp 100 ml NEW` parses as `Maison Alhambra / Tonic Ice / Eau de Parfum / 100ml` with manual review.
22. Build `MatchGroup` records for likely same-product rows.
23. Staff approve/reject/link through manual decisions or bulk actions.
24. Persist links to `SupplierProduct.catalog_perfume` and optional `catalog_variant`; audit decisions.

External catalogue import:
1. Parse saved HTML or parsed Fragrantica catalogue JSON/CSV into staged `FragranticaProduct` rows, or manually add a single staged `FragranticaProduct` row from the Fragrantica Products review page when an operator needs one missing source product.
...
4. Review staged rows beside local catalogue rows. Suggested matches should be ranked and should use catalogue brand/product aliases, approved regex preprocess rules, loose punctuation keys, connector synonyms such as `&`/`and`/`et`, compact brand keys such as `Dolce&Gabbana`/`Dolce & Gabbana`, audience synonyms and suffixes such as `wom`, `man`/`men`, `woman`/`women`, `Pour Femme`, and `Pour Homme`, removable concentration phrases embedded in source scent titles such as `Light Blue Eau de Toilette`, and concentration aliases such as title suffixes `Extrait`, `Parfum`, `Perfume`, or `Pure Perfume` meaning `Extrait de Parfum`, plus fuzzy same-brand scent scoring. Preserve audience-bearing scent suffixes while removing concentration words before falling back to looser audience cleanup. If a staged Fragrantica title explicitly names a concentration, treat matching concentration as stronger exact evidence for ranking, but keep a source title that omits concentration as an exact brand+scent candidate when identity otherwise matches; if both a concentration-specific source and a generic source compete for the same concentrated local perfume, demote the generic source below `100 only`; cap contradictory concentration matches below exact confidence. Strip a repeated leading brand name from the staged Fragrantica scent identity for display, matching, and reviewed apply actions; for matching only, also try a source-name variant with a repeated leading source collection removed. Fragrantica product search must match all query tokens across staged fields together, including brand, normalized brand, collection, scent, normalized scent, audience, year, source path, URL, and domain. Reject opposite-audience candidates when both sides have known men/women audience. Apply product aliases after the same identity cleanup so alias rows still match when source titles include concentration or approved supplier abbreviations. Treat identity equality after this cleanup as an exact candidate; do not limit suggestions to exact raw normalized names. Keep manual-review conflict candidates separate from numeric confidence filters: a score-100 candidate that needs review should appear under a needs-review filter, not under `100 only`.
5. Link staged rows to local `catalog.Perfume` records after review; keep local concentration and variants. Do not copy concentration words embedded in the Fragrantica scent title into the local scent name; use them only as match evidence. Treat Latin diacritics in Fragrantica brand/scent text as match-equivalent to plain English letters (`ò` = `o`, `ì` = `i`, `é` = `e`, etc.), and treat apostrophe variants as match-equivalent separators (`L’air`, `L'air`, and `L air` match), but fold them to catalogue-safe ASCII before storing reviewed local catalogue names. Reviewed local names should preserve a straight apostrophe where the source has one, such as `L'air Barbes`. Linked rows are no longer suggestion candidates for the linked local product; review UI should show the existing linked counterpart only. Treat Fragrantica linking as one reviewed Fragrantica row to one Our Products perfume by default: when the same unlinked Fragrantica row is an equal top match for multiple local perfumes, remove it from bulk-ready linking and force manual review rather than auto-linking duplicates. Rarely, one Fragrantica row may be manually approved for a second Our Products perfume; keep this as a reviewed extra link rather than changing the primary `matched_perfume`. Staff may explicitly unlink either the primary Fragrantica match or a rare reviewed extra link from the linking workbench; unlinking removes only the selected reviewed relationship and does not delete the local product or staged Fragrantica row. All-page bulk linking is stricter than checked-row bulk linking: it may apply only current-filter ready suggestions at `95+` or `100 only`, must skip manual-review rows and duplicate Fragrantica targets, and should run as a queued/background job in production when it can span thousands of rows. When applying Fragrantica collection or all-uppercase scent names to a local product, normalize source casing to catalogue title case: major words capitalized, known short words such as `for`/`and` lowercased after the first word, and approved short acronyms such as `WB` preserved. If a brand has same-base men/women scents in local catalogue or staged Fragrantica rows, the reviewed local scent name must keep audience in the name while preserving the separate audience field: use `Pour Homme`/`Pour Femme` when the sibling set uses that style, otherwise use `for Men`/`for Women`. Do not surface a generic no-audience local base scent as a manual-review Fragrantica candidate when the brand already has audience-specific local siblings for that same base and the Fragrantica source is explicitly Men or Women.
...
8. Use the Our Products concentration audit as a review queue, not an auto-cleaner. It may flag local names containing concentration text, linked Fragrantica titles that disagree with the local concentration, and same-base scents split across multiple concentrations without explicit source proof; staff must still decide whether to update, merge, delete, or mark reviewed/conflict.
...
- In the Fragrantica linking workbench, an individual staff click on a candidate's `Link` button is the manual review approval for that one row. Bulk linking must remain stricter and skip manual-review rows, already-linked duplicate targets, and unsafe duplicate Fragrantica targets.
