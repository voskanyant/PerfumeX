...
4. Review staged rows beside local catalogue rows. Suggested matches should be ranked and should use catalogue brand/product aliases, loose punctuation keys, compact brand keys such as `Dolce&Gabbana`/`Dolce & Gabbana`, audience synonyms and suffixes such as `man`/`men`, `woman`/`women`, and `Pour Femme`, removable concentration phrases embedded in source scent titles such as `Light Blue Eau de Toilette`, and fuzzy same-brand scent scoring. Treat identity equality after this cleanup as an exact candidate; do not limit suggestions to exact raw normalized names.
