...
- 2026-05-06 - Performance: Catalogue linking `95+` and `Needs review` filters must not verify every filtered row before first render on live data. Keep the exact prefilter, then hydrate only a bounded page scan; reserve full verified pagination/counts for `100 only`.
