...
- 2026-05-04 - Performance: Catalogue linking workbench rows should reuse server-rendered visible-row candidate payloads on selection. Do not re-query the candidate endpoint when the selected row already has hydrated matches; keep AJAX only as a fallback for missing payloads.
...
- 2026-05-05 - Performance: Catalogue linking strict exact filters (`100 only` with suggestions) should prefilter the queryset to exact Fragrantica match IDs before pagination, then score only the visible page; do not scan arbitrary catalogue windows that return underfilled pages.
- 2026-05-05 - Performance: Supplier Products AJAX search is the reference pattern for fast live search: avoid exact full counts and expensive row hydration, use indexed filters, cheap deterministic ordering, fetch `page_size + 1` rows for pagination, and lazy-load heavy details only when the user asks for them.
