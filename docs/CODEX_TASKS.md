...
- 2026-05-24 - Performance: Normalization product detail GETs must stay read-only and bounded. Do not save/reparse stale parses or scan every similar supplier row while opening the page; use explicit Reparse actions and capped previews with a `+` indicator when more rows exist.
...
- 2026-05-24 - Performance: Catalogue linking should keep ordinary unscored pages exact-count paginated, but use a short-lived per-session scan cache for scored suggestion/confidence filters so page 2/3 continue from already scanned rows instead of rescanning from row 1.
- 2026-05-26 - Catalogue linking: high-confidence Fragrantica source-name prefilters must stay per perfume after batch fetches. A source admitted because it exactly matches one perfume must not be scored as a 95+/98 suggestion for another same-brand perfume in the same scan batch.
- 2026-05-31 - Performance: Catalogue linking broad scored filters may have sparse true matches after false positives are removed. If the first bounded scan finds no filtered rows, continue scanning just enough to find a first real match or prove exhaustion so the UI does not show a false empty page while exact totals hydrate.
...
- 2026-05-07 - Performance: Catalogue linking workbench scored filters should avoid Django's exact-count paginator on broad pages. Use count-free `page_size + 1` pagination and show count-free row summaries unless the filtered scan is exhausted or an exact scoped path is deliberately chosen.
