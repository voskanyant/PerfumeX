...
- When parser rules, aliases, catalogue-backed normalization behavior, or seeded assistant knowledge change saved parse output, bump `PARSER_VERSION` and run a bounded production reparse for the affected supplier-name patterns; full-table reparses are too large for the deploy SSH timeout.
