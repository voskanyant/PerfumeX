...
- Our Products linked Fragrantica rows should stay compact: show catalogue collection/year/audience as one metadata row under the product name, render the linked Fragrantica counterpart as inline green text, and avoid linked-row background fills, nested linked-match cards, a separate flags column, or repeated informational chips.
