...
class Command(BaseCommand):
    help = "Parse a saved or parsed brand catalogue file into the external Fragrantica staging catalogue."

    def add_arguments(self, parser):
        parser.add_argument(
            "path",
            help="Path to saved HTML/text/JSON/CSV from a brand catalogue page.",
        )
        parser.add_argument("--brand", help="Override detected brand name.")
        parser.add_argument(
            "--source-url",
            default="",
            help="Original source URL to attach to staged Fragrantica rows.",
        )
        parser.add_argument(
            "--apply",
            action="store_true",
        ...
        parser.add_argument(
            "--missing-report", help="Write missing catalogue items to a CSV file."
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        if options["create_missing_catalog"]:
            raise CommandError(
                "--create-missing-catalog is disabled. Import Fragrantica rows first, then merge after review."
            )
        if options["create_aliases"]:
            raise CommandError(
                "--create-aliases is disabled. Link/merge Fragrantica rows before creating normalizer knowledge."
            )
        if (
            options["reparse_supplier_products"]
            or options["reparse_all_supplier_products"]
        ):
            raise CommandError(
                "HTML import only stages Fragrantica rows. Reparse after approved link/merge knowledge changes."
            )
        ...
        for item in summary.source_items:
            if item.audience:
                audience_counts[item.audience] = (
                    audience_counts.get(item.audience, 0) + 1
                )
        if audience_counts:
        ...
        self.stdout.write(
            f"Existing Fragrantica products: {len(summary.existing_fragrantica_products)}"
        )
        self.stdout.write(f"New Fragrantica products: {len(summary.missing_items)}")
        self.stdout.write(
            f"Created Fragrantica products: {len(summary.created_fragrantica_products)}"
        )
        self.stdout.write(
            f"Updated Fragrantica products: {len(summary.updated_fragrantica_products)}"
        )
        if options["missing_report"]:
