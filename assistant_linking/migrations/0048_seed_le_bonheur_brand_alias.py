...
BRAND_NAME = "Le Bonheur"
BRAND_ALIASES = (
    ("\u041b\u0435 \u0411\u043e\u043d\u0435\u0440", 20),
    ("Le Bonheur", 20),
    ("Le Bonheur Perfumes", 20),
)
...
def seed_le_bonheur_brand_alias(apps, schema_editor):
...
def unseed_le_bonheur_brand_alias(apps, schema_editor):
    Brand = apps.get_model("catalog", "Brand")
    BrandAlias = apps.get_model("assistant_linking", "BrandAlias")
    ...
    if not brand.perfumes.exists() and not brand.aliases.exists():
        brand.delete()


class Migration(migrations.Migration):
    dependencies = [
        ("assistant_linking", "0047_seed_cosmogony_brand_alias"),
    ]

    operations = [
        migrations.RunPython(seed_le_bonheur_brand_alias, unseed_le_bonheur_brand_alias),
    ]
